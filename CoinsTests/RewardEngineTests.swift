...
final class RewardEngineTests: XCTestCase {
    func testLockoutBlocksRapidTaps() {
        ...
        let first = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: now)
        let second = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: now.addingTimeInterval(30))
    ...
    func testRepetitionPresetAndConfiguredDailyStreakRewardsStack() {
        ...
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: dayOne, calendar: calendar)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: dayOne.addingTimeInterval(31), calendar: calendar)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: dayOne.addingTimeInterval(62), calendar: calendar)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: dayTwo, calendar: calendar)
        let result = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: dayThree, calendar: calendar)

        XCTAssertEqual(snapshot.state.streakLevel(for: "concert-streak"), 3)
    ...
    func testDailyMaximumBlocksAdditionalCompletionsUntilNextDay() {
        ...
        let first = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: start, calendar: calendar)
        ...
        XCTAssertFalse(first.isDenied)
        XCTAssertTrue(second.isDenied)
        XCTAssertEqual(second.deniedReason, "Daily maximum reached for Concert Piece.")
    ...
    func testStreakIgnoresActivitiesOutsideConfiguredSet() {
        ...
        snapshot.config.streaks = [
            StreakDefinition(
                id: "concert-only",
                title: "Concert Piece Only",
                detail: "Only concert-piece completions count.",
                activityIDs: ["concert-piece"],
                dailyMinimum: 1,
                bonusPreset: .noBreaks
            )
        ...
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let result = RewardEngine.complete(activityID: "review-piece", snapshot: &snapshot, now: now)
    ...
    func testDailyMinimumMustBeMetBeforeStreakUpdates() {
        ...
        snapshot.config.streaks = [
            StreakDefinition(
                ...
                activityIDs: ["concert-piece", "review-piece", "violin-exercise"],
                dailyMinimum: 3,
                bonusPreset: .noBreaks
            )
        ]
        ...
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: dayOne, calendar: calendar)
        _ = RewardEngine.complete(activityID: "review-piece", snapshot: &snapshot, now: dayOne, calendar: calendar)
        _ = RewardEngine.complete(activityID: "violin-exercise", snapshot: &snapshot, now: dayOne, calendar: calendar)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: dayTwo, calendar: calendar)
        _ = RewardEngine.complete(activityID: "review-piece", snapshot: &snapshot, now: dayTwo, calendar: calendar)
        let result = RewardEngine.complete(activityID: "violin-exercise", snapshot: &snapshot, now: dayTwo, calendar: calendar)
    ...
    func testCashOutUsesWatermarkInsteadOfReducingBalance() {
        ...
        _ = RewardEngine.complete(activityID: "violin-exercise", snapshot: &snapshot, now: start)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: start.addingTimeInterval(301))
    ...
    func testCompletionRecordsActivityHistoryAndRewardProvenance() {
        ...
        XCTAssertEqual(snapshot.state.activityEvents.count, 1)
        XCTAssertEqual(snapshot.state.activityEvents[0].activityID, "review-piece")
        XCTAssertEqual(snapshot.state.activityEvents[0].activityTitle, "Review Piece")
    ...
    func testHistoryRewriteCutsLaterTimestampsFirstAndAddsManualEvents() {
        ...
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: day.addingTimeInterval(60), calendar: calendar)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: day.addingTimeInterval(120), calendar: calendar)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: day.addingTimeInterval(180), calendar: calendar)

        snapshot.state.rewriteActivityHistory(
            on: day,
            countsByActivityID: ["concert-piece": 1],
            activities: snapshot.config.activities,
            calendar: calendar
        )

        XCTAssertEqual(snapshot.state.activityCounts(on: day, calendar: calendar)["concert-piece"], 1)
        XCTAssertEqual(snapshot.state.activityEvents.first?.createdAt, day.addingTimeInterval(60))

        snapshot.state.rewriteActivityHistory(
            on: day,
            countsByActivityID: ["concert-piece": 3],
            activities: snapshot.config.activities,
            calendar: calendar
        )

        let concertPieceEvents = snapshot.state.activityEvents.filter { $0.activityID == "concert-piece" }
        XCTAssertEqual(concertPieceEvents.count, 3)
        XCTAssertEqual(concertPieceEvents.filter(\.isManuallyAdded).count, 2)
        XCTAssertEqual(concertPieceEvents.compactMap(\.timestamp), [day.addingTimeInterval(60)])
    }
    ...
    func testRewindRecordsActualDeltaWithoutTakingBalanceNegative() {
        var snapshot = GameSnapshot.seed
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: now)
    ...
    func testSnapshotRoundTripPreservesEventHistories() throws {
        ...
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: now)
        _ = RewardEngine.complete(activityID: "concert-piece", snapshot: &snapshot, now: now.addingTimeInterval(30))
